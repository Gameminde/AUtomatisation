    ...
    try:
        ...
        if metrics.get("reach", 0) > 0:
            metrics["engagement_rate"] = (
                metrics.get("engaged_users", 0) / metrics["reach"]
            ) * 100
        else:
            metrics["engagement_rate"] = 0.0
        
        logger.info(f"📊 Métriques {post_id}: reach={metrics.get('reach', 0)}, engagement={metrics.get('engagement_rate', 0):.2f}%")
        return metrics
...
def get_best_performing_topics(limit: int = 10) -> List[Dict]:
    ...
    try:
        ...
        result = client.table("post_analytics") \
...
def get_daily_stats() -> Dict:
    ...
    try:
        ...
        # Articles collectés
        raw = client.table("raw_articles") \
            .select("id", count="exact") \
            .gte("scraped_at", cutoff) \
            .execute()
        
        # Contenus générés
        processed = client.table("processed_content") \
            .select("id", count="exact") \
        ...
        stats = {
            ...
            "content_generated": processed.count if hasattr(processed, 'count') else len(processed.data or []),
            "posts_published": published.count if hasattr(published, 'count') else len(published.data or []),
            "images_created": processed.count if hasattr(processed, 'count') else len(processed.data or []),
